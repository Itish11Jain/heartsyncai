...
/**
 * Warm the model at server startup so the first user request is fast.
 * Safe to call without awaiting; errors are logged, not thrown.
 */
export function warmBgRemove(): void {
  load().catch((err) => {
    console.error("[bgRemove] warm-up failed (will retry on first request)", err);
  });
}
 ...
 *
 * Inference is serialized across requests to bound memory/CPU usage.
 *
