...
function SenderPanelInner({ senderShareUrl, recipientName, occasion, cardId, phase }: SenderPanelProps) {
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      // Use classic resource API: authenticateWithRedirect is stable across Clerk versions.
      // signIn.sso (signal wrapper) calls target().sso() which doesn't exist on the classic
      // SignInResource → silently does nothing. Direct classic API is the reliable path.
      type ClassicSignIn = {
        authenticateWithRedirect: (p: {
          strategy: string; redirectUrl: string; redirectUrlComplete: string;
        }) => Promise<void>
      };
      const clerkAny = clerk as unknown as { client?: { signIn?: ClassicSignIn } };
      const classicSignIn = clerkAny.client?.signIn as ClassicSignIn | undefined;

      if (classicSignIn?.authenticateWithRedirect) {
        await classicSignIn.authenticateWithRedirect({
          strategy: "oauth_google",
          redirectUrl: returnUrl.toString(),
          redirectUrlComplete: returnUrl.toString(),
        });
      } else {
        // Fallback: try signal API sso()
        await (signIn as unknown as {
          sso: (p: { strategy: string; redirectUrl: string; redirectUrlComplete: string }) => Promise<void>
        }).sso({
          strategy: "oauth_google",
          redirectUrl: returnUrl.toString(),
          redirectUrlComplete: returnUrl.toString(),
      ...
      console.error("[HeartSync] Google sign-in error:", err);
      const e = err as { errors?: Array<{ longMessage?: string; message: string }> };
      const msg = e.errors?.[0]?.longMessage ?? e.errors?.[0]?.message;
      setSignInError(msg ?? "Google sign-in failed — try email code instead.");
            ...
            >
              ...
              {signInStep === "email" ? (
                <>
                  ...
                  >
                    {googleLoading ? (
                      <Loader2 style={{ width: 18, height: 18, animation: "spin 1s linear infinite", color: "#6b7280" }} />
                    ) : (
