 ...
 *   hook       — glowing crystal ball; title + rub-to-clear instruction
 *   clearing   — pointer-drag clears swirling mist (280px target)
...
/* ── Safe-zone quadrant layout (same pattern as cosmic.tsx) ── */
const SAFE_ZONES = [
  { cx: 25, cy: 22 },
  { cx: 72, cy: 22 },
  { cx: 25, cy: 68 },
...
/* ── Crystal ball ambient canvas — swirling mist / galaxy ── */
function CrystalCanvas({
  ...
  useEffect(() => {
    ...
    /* Seed mist particles inside ball */
    function seedMist() {
    ...
    /* Seed star field for galaxy mode */
    function seedStars() {
    ...
    function draw() {
      ...
      if (mode === "mist") {
        /* Swirling mist particles inside ball */
        const ball = ballRef.current;
        if (ball) {
          const rect = ball.getBoundingClientRect();
          const cx = rect.left + rect.width / 2;
          ...
          /* Shimmer veins — faint radial lines */
          if (mistOpacity > 0.05) {
            ctx.save();
            ctx.beginPath();
            ctx.arc(cx, cy, r * 0.93, 0, Math.PI * 2);
        ...
        /* Galaxy mode — full screen star field + shooting stars */
...
/* ── Prismatic shard burst on tap ── */
function ShardBurst({ x, y, onDone }: { x: number; y: number; onDone: () => void }) {
  useEffect(() => {
  ...
  return (
    <motion.div
      style={{ position: "fixed", left: x, top: y, width: 0, height: 0, zIndex: 25, pointerEvents: "none" }}
    >
      {shards.map((s, i) => (
        <motion.div
          key={i}
          initial={{ opacity: 0.95, scaleX: 0.2, rotate: s.angle, x: 0, y: 0 }}
          animate={{
            opacity: 0,
            scaleX: 1,
            x: Math.cos((s.angle * Math.PI) / 180) * s.len,
            y: Math.sin((s.angle * Math.PI) / 180) * s.len,
          }}
          transition={{ duration: 0.55 + s.delay, ease: "easeOut" }}
          style={{
            position: "absolute",
            width: s.len,
            height: 2.5,
            borderRadius: 2,
            background: `linear-gradient(90deg, hsl(${s.hue},100%,75%), hsl(${(s.hue + 60) % 360},100%,80%))`,
...
export default function CrystalCard() {
  ...
  const tpl =
    getCrystalTemplate(occasion, relation) ?? getCrystalFallback(occasion);
  const finalMessage = (() => {
    try {
  ...
  /* Auto-advance from hook to clearing */
  useEffect(() => {
  ...
  const handleOrbTap = useCallback(
    (idx: number, e: React.MouseEvent | React.TouchEvent) => {
      ...
      tooltipTimer.current = setTimeout(() => setTooltip(null), 3200);

      /* Advance to revelation when all 4 tapped */
      setTappedOrbs((prev) => {
        const next = new Set(prev);
        next.add(idx);
  ...
  function shareSenderWhatsApp() {
    trackEvent({ event: "card_shared", channel: "whatsapp", template: "crystal" });
  ...
  async function copySenderLinkForInstagram() {
    trackEvent({ event: "card_shared", channel: "instagram", template: "crystal" });
  ...
  async function copySenderLink() {
    trackEvent({ event: "card_shared", channel: "copy", template: "crystal" });
    crystalHaptics.copy();
    await navigator.clipboard.writeText(cardUrl()).catch(() => {});
  ...
  /* Pulse ring spring for clearing phase */
  const rubPulse = useSpring(0, { stiffness: 200, damping: 20 });
  const ringScale = useTransform(rubPulse, [0, 1], [1, 1.12]);

  useEffect(() => {
    if (phase !== "clearing") return;
    const interval = setInterval(() => {
      rubPulse.set(1);
      setTimeout(() => rubPulse.set(0), 320);
    }, 1800);
    return () => clearInterval(interval);
  }, [phase, rubPulse]);

  /* Reveal shimmer for visions phase */
  const visionsBg =
    "radial-gradient(ellipse at 50% 45%, rgba(90,30,160,0.75) 0%, rgba(20,6,50,0.98) 70%)";
  const galaxyBg =
  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        touchAction: phase === "clearing" ? "none" : "auto",
        userSelect: "none",
      }}
    ...
    >
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            "radial-gradient(ellipse 80% 60% at 50% 55%, rgba(100,40,200,0.12) 0%, transparent 70%)",
          pointerEvents: "none",
          zIndex: 0,
        }}
      />

      {/* Canvas overlay (mist in clearing, galaxy in revelation) */}
      {(phase === "clearing" || phase === "visions" || phase === "revelation") && (
        <CrystalCanvas
          ...
          >
            ...
            <motion.div
              ...
              style={{
                ...
                borderRadius: "50%",
                position: "relative",
                cursor: phase === "clearing" ? "grab" : "default",
                scale: phase === "clearing" ? ringScale : 1,
                flexShrink: 0,
              }}
            >
              {/* Outer glow ring */}
              <motion.div
                animate={{
                  boxShadow: [
                    "0 0 40px 10px rgba(130,60,240,0.35)",
                    "0 0 70px 20px rgba(160,90,255,0.55)",
                    "0 0 40px 10px rgba(130,60,240,0.35)",
                  ],
                }}
              ...
              >
                ...
                <motion.div
                  ...
                  style={{
                    ...
                    borderRadius: "50%",
                    background:
                      "radial-gradient(circle, rgba(180,120,255,0.45) 0%, transparent 70%)",
                  }}
                />
                ...
                <div
                  style={{
                    ...
                    borderRadius: "50%",
                    background:
                      "radial-gradient(circle at 50% 55%, rgba(140,100,220,0.22) 0%, rgba(60,20,120,0.18) 60%, transparent 100%)",
            ...
            >
              <div
                style={{
                  ...
                  height: 10,
                  background:
                    "linear-gradient(90deg, transparent, rgba(160,120,255,0.35), transparent)",
                  borderRadius: 999,
                }}
              />
              <div
                style={{
                  ...
                  height: 18,
                  background:
                    "linear-gradient(180deg, rgba(140,100,220,0.28) 0%, rgba(80,40,160,0.18) 100%)",
              ...
              <div
                style={{
                  ...
                  height: 8,
                  background:
                    "linear-gradient(90deg, transparent, rgba(160,120,255,0.22), transparent)",
                  borderRadius: 999,
                  marginTop: 1,
                }}
              />
            ...
            <AnimatePresence>
              {phase === "clearing" && (
                <motion.p
                  ...
                  exit={{ opacity: 0 }}
                  transition={{ duration: 2.2, times: [0, 0.2, 0.7, 1], repeat: Infinity }}
                  style={{
                    marginTop: 28,
            ...
            {/* Hook pulse ring (clearing phase) */}
            <AnimatePresence>
              {phase === "clearing" && (
                <motion.div
                  key="pulse"
              ...
              return (
                <motion.button
                  ...
                  style={{
                    ...
                    borderRadius: "50%",
                    border: tapped
                      ? "2px solid rgba(220,200,255,0.65)"
                      : "1.5px solid rgba(180,140,255,0.4)",
                    background: tapped
                    ...
                    boxShadow: tapped
                      ? "0 0 20px rgba(180,130,255,0.55), inset 0 0 12px rgba(255,255,255,0.08)"
      ...
      {bursts.map((b) => (
        <ShardBurst
          ...
          x={b.x}
          y={b.y}
          onDone={() => setBursts((prev) => prev.filter((s) => s.id !== b.id))}
        />
      ))}
          ...
          >
            ...
            <motion.div
              ...
              style={{
                ...
                background:
                  "linear-gradient(145deg, rgba(42,12,80,0.98) 0%, rgba(16,4,44,0.99) 100%)",
                borderRadius: 26,
                border: "1.5px solid rgba(190,150,255,0.45)",
                padding: "38px 30px",
                textAlign: "center",
                boxShadow:
                  "0 0 60px rgba(130,60,240,0.38), 0 0 140px rgba(100,40,200,0.18), inset 0 1px 0 rgba(255,255,255,0.1)",
                position: "relative",
                overflow: "hidden",
              }}
            >
              ...
              <motion.div
                ...
                style={{
                  ...
                  height: "100%",
                  background:
                    "linear-gradient(105deg, transparent, rgba(200,160,255,0.06), rgba(255,220,200,0.04), transparent)",
                  pointerEvents: "none",
                }}
              />
              ...
              <div
                style={{
                  ...
                  height: "60%",
                  background:
                    "radial-gradient(ellipse, rgba(120,50,230,0.12) 0%, transparent 70%)",
                  pointerEvents: "none",
                }}
              />

              {/* Crystal emoji header */}
              <motion.div
                initial={{ opacity: 0, scale: 0.7 }}
                animate={{ opacity: 1, scale: 1 }}
              ...
              {/* Sender credit hidden; sender sees share panel below */}
              {isRecipient && (
                <motion.p
                  initial={{ opacity: 0 }}
                ...
                <div style={{ display: "flex", gap: 10, marginBottom: 8 }}>
                  <button
                    ...
                    style={{
                      ...
                      border: "1.5px solid rgba(37,211,102,0.26)",
                      color: "rgba(37,211,102,0.88)",
                      fontWeight: 700,
                      fontSize: 13,
                      cursor: "pointer",
                    }}
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: "1.5px solid rgba(200,100,200,0.26)",
                      color: "rgba(220,140,255,0.88)",
                      fontWeight: 700,
                      fontSize: 13,
                      cursor: "pointer",
                    }}
