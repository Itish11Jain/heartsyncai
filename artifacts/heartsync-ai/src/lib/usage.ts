...
export function useCardUsage() {
  ...
  const fetchUsage = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE}/api/usage/check?fingerprint=${encodeURIComponent(fingerprint)}`,
        { headers }
      );
      if (res.ok) {
      ...
      setUsage({ anon_used: 0, signed_in_used: 0, is_signed_in: !!isSignedIn });
    } finally {
      setLoading(false);
    }
  }, [isSignedIn, isLoaded, fingerprint, getToken]);
  ...
  }, [isSignedIn, fingerprint, fetchUsage, getToken]);

  return { usage, loading, fingerprint, incrementUsage, refetch: fetchUsage };
}
