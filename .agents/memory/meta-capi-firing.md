...
### Server CAPI fires on BOTH paths (webhook AND verify), NOT one-shot
`fireMetaCapi` is intentionally OUTSIDE the `fireOnce` claim gate in `fulfillCardUnlock` — it runs on every fulfillment call (webhook + verify + retries). This is safe because Meta dedups on the shared `event_id`.
**Why:** the Razorpay webhook wins the `fireOnce` race but carries NO `fbp`/`fbc`/ip/ua (server-to-server, no buyer cookies), so a one-shot CAPI fired an EMPTY event that Meta rejected (2804050) and the gate then suppressed the data-rich `/verify` event. Confirmed in prod: 8/9 rejected cards actually HAD `fbp`+`fbc` in `hs_cards` — `/verify` backfilled them a moment too late. The empty webhook event being *rejected* means it never enters Meta's dedup pool, so the later accepted `/verify` event (with `fbp`/`fbc`) lands instead of being dropped as a duplicate.
**How to apply:** keep `fireMetaCapi` out of `fireOnce`. ONLY the `card_paid` analytics insert (and the "card unlocked" log) stay one-shot — that row is NOT event_id-deduped and would double-count a single sale across the two paths. Never re-gate the CAPI call behind `fireOnce` "to avoid duplicates" — dedup is Meta's job via `event_id`, and gating it reintroduces the empty-event bug.

### GOTCHA: the webhook is the path that must read per-order CAPI data
The Razorpay **webhook usually WINS the fireOnce race** (server-to-server, fires before the buyer's `/verify`). So any per-order field the server CAPI needs — `event_id`, and to a lesser extent fbp/fbc — must be selected by the **webhook's** order query, not just verify's. There are THREE spots that touch the order row in razorpay.ts: the create-order INSERT, the `/verify` SELECT, and the `/webhook` SELECT — and the two SELECTs have DIFFERENT indentation, so a `replace_all` keyed on whitespace silently edits only one. A first cut of this fix added `event_id` to create-order + verify but missed the webhook SELECT, so `order.event_id` came back `undefined` on the winning path and `fulfillCardUnlock` fell back to the synthetic id — the dedup looked "done" (tsc passed) but production kept firing synthetic ids (visible as `[capi] ... event_id=hs_<card>_<servertime>` REJECTED 2804050 in deployment logs even though the prod DB showed the order had event_id and the card had fbp/fbc).
**Why:** the symptom is invisible in dev/tsc and only shows in production deployment logs; the diagnostic tell is the embedded timestamp in the logged event_id being ~the fire time (server) rather than ~create-order time (browser).
**How to apply:** whenever you add a column the CAPI/fulfillment path consumes, grep ALL `FROM hs_razorpay_orders` SELECTs and confirm EACH includes it; never trust a whitespace-keyed replace_all across the two SELECTs. Verify in prod logs that webhook-won Purchases carry the browser-time event_id, not a fresh server-time one.
